<template>
  <div class="app-container">
    ...
    <el-table :data="tableList" v-loading="listLoading" border element-loading-text="拼命加载中" style="width: 100%;">
      ...
      <el-table-column prop="tagName" label="姓名">
      </el-table-column>
      <el-table-column prop="tagDesc" min-width="150px" label="标题">
      </el-table-column>
      <el-table-column prop="tagDesc" label="阅读数"  width="65">
      </el-table-column>
    ...
    updateData() {
      ...
      updateArticle(tempData).then(() => {
        for (const v of this.tableList) {
          if (v.uid === this.temp.uid) {
            const index = this.tableList.indexOf(v)
            this.tableList.splice(list, 1, this.temp)
            break
          }
        }
